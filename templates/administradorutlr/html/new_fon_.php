...
*/	
	...
	if($row_num == 0){// Cuando no existe ningun registro
		...
		$row_ent = mysql_fetch_array($sql_en);
		$id_ent = $row_ent[0];
	}
	...
	*/	
	...
	while($row_inf_tip_tit = mysql_fetch_array($sql_inf_tip_tit)){
	...
	/*traslado de la info de tipo de fondo a fondo de la entidad*/
		...
		if($id_pol_actual != $row_inf_tip_tit[4]){ // -- Si el id de politicas es diferente procede a crear una nueva politica
			...
			*/
			while($row_inf_tip_comp = mysql_fetch_array($sql_inf_tip_comp)){
...
//----/// Prohibiciones
  ...
  /
	...
	while($row_co_tfn = mysql_fetch_array($sql_co_tfn)){
