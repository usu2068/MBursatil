...
$nits_emis = json_decode($_POST['ids_emis'], true);
...
if($c_inv != 0 || $c_dep != 0){		
	...
	for($i = 1; $i<=count($nits_emis); ++$i){
		if($sel_inv[$i] != '' || $sel_dep[$i] != ''){
			...
			$row_emis = mysqli_fetch_array($sql_emis);
			
			$table_fond = array('jo33_FIC_UTLR_fondos');
			$valC_fond = array('codigo', 'active', 'trash');
			$val_fond = array($cod_fon, 1, 1);
			$sql_fond = $consult -> sql('S', $table_fond, $val_fond, $valC_fond, $valU);
			$row_fond = mysqli_fetch_array($sql_fond);
			...
			$row_emiXfon = mysqli_fetch_array($sql_emiXfon);
		...
		// GUARDA VALORES DE LOS EMISORES POR DEPOSITOS
			if($sel_dep[$i] != ''){
				...
				if(count($row_emiXfon) <= 1){
					...
					$valC_emiXfon = array('id_emisores','id_fondos','max_dep');
