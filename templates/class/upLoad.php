...
/*
	...
	*
	...
	while($row = mysqli_fetch_array($result)){
		...
		$row_ult = mysqli_fetch_array($result_ult);
...
if ($_FILES[arch_dep][size]>25000000){
	$msg=$msg."Uno de los archivos es mayor que 25Mg, debes reduzcir el peso antes de subirlo";
	//$upload_351="false";
	$upload_depo="false";
}
...
function analisis($ruta, $id_ent, $emisores_pdf_inv, $emisores_pdf_dep, $fecha_fonds, $grp_tot){
	...
	*/
	...
	$row_smlv = mysqli_fetch_array($sql_smlv);
	...
	*/
	...
	while($row_comp = mysqli_fetch_array($sql_comp)) 
		$composicion[$row_comp[0]] = $row_comp[1];
	...
	while($row_tot = mysqli_fetch_array($sql_tot)) 
		$totales[$row_tot[0]] = $row_tot[1];
	...
	*/	
	...
	while($row_proh = mysqli_fetch_array($sql_proh)){
	...
	while($row_sql = mysqli_fetch_array($sql)){
		...
		if($row_sql[13] != $cod_fon && $row_sql[13] != ""){
			...
			*/
			...
			while($row_grpeco = mysqli_fetch_array($sql_grpeco)){
			...
			for($f=0; $f<count($nom_grpeco); ++$f){
				...
				$row_grpecoXfon = mysqli_fetch_array($sql_grpecoXfon);
			...
			// CONSTRUCION DE LA TABLA HTML DE LOS GRUPOS ECONOMICOS POR DEPOSITOS
				...
				if($grp_eco_dep[$cont_grp_eco][$f] != 0){
		...
		if($row_sql[41] == 'tot'){
			...
			if($id_tot != $row_sql[36]){
				...
				if(	$totales[$Nom_ant] == 'Total Bonos' ||
					...
					utf8_encode($totales[$Nom_ant]) == 'Total Nación' ||
					
					$totales[$Nom_ant] == 'Total Otros Renta Fija' ||
					$totales[$Nom_ant] == 'Total Acciones' ||

					utf8_encode($totales[$Nom_ant]) == 'Total Participación en Fondos de Inversión Colectiva' ||
					utf8_encode($totales[$Nom_ant]) == 'Total Derechos Económicos' ||
					
					$totales[$Nom_ant] == 'Total Otros Renta Variable' ||
					$totales[$Nom_ant] == 'Total Renta Variable' ){
...
/*********************************
			print_r($valor_total);
			echo '<br> antes de suma';
			
			$valor_total = $valor_total + $valor_actual;

			print_r($valor_total);
			echo '<br> despues de suma';
/*********************************/
	...
	*/
			...
			for($f=0; $f<count($nom_grpeco); ++$f){
				...
				$table_grpecoXfon = array('jo33_FIC_UTLR_fondos_X_grupos_eco');
				$valC_grpecoXfon = array('id_fondos', 'id_grupos_eco');
				$val_grpecoXfon = array('"'.$id_fondos.'"', '"'.$id_grpeco[$f].'"');				
				$sql_grpecoXfon = $consult -> sql('S', $table_grpecoXfon, $val_grpecoXfon, $valC_grpecoXfon, $valU);
	...
	*/
	...
	$table = $header_fon.$table_prom.$table.$table_cal.$table_grp_eco;
