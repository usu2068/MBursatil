	...
	class listado{
		...
		function tablas($table_lis, $id_cat_pri, $parent, $tip){
			...
			if($tip == 'E'){
				...
				while($row_list = mysql_fetch_array($sql_list)){
					...
					$row_usuXent = mysql_num_rows($sql_usuXent);
					...
					if($row_usuXent == 0)
						...
						while($row_usuXent = mysql_fetch_array($sql_usuXent)){
		...
		function frm_edit($tip, $id){
			...
			if($tip == 'E'){
				...
				$row_ent = mysql_fetch_array($sql_ent);
		...
		function tipo_fondo($tab_sql, $tip, $id_entJ){
				...
				while($row_tip = mysql_fetch_array($sql_tip)){
		...
		*/
			...
			while($row_pr_fn = mysql_fetch_array($sql_pr_fn)){
			...
			while($row_nom_comp = mysql_fetch_array($sql_nom_comp)){
			...
			while($row_nom_tit = mysql_fetch_array($sql_nom_tit)){
		...
		//consulta la existencia de prohibiciones para el tipo de fondo
			...
			//consulta la existencia de compociciones para las prohibiciones
				...
				$row_num = mysql_num_rows($sql_pr_com);
				...
				//consulta la existencia de titulos para las compociciones
					...
					while($row_pr_com = mysql_fetch_array($sql_pr_com)){
					...
					/
						...
						$row_num = mysql_num_rows($sql_pr_tit);
						...
						if($row_num != 0){
							...
							while($row_pr_tit = mysql_fetch_array($sql_pr_tit)){
		...
		function politicas($tip, $id){
			...
			*/
			...
			$num_cal = 0;
			$com = 0;
			//$row_next = mysql_fetch_array($sql_pol_II);
			
			while($row_pol = mysql_fetch_array($sql_pol)){
			...
			$js_guarda = "href='javascript:guard_para_pol($tip, ".json_encode($idst).", ".json_encode($idto).", $id);'";
		...
		function paginacion($sql_tit_asig, $tip){
			...
			if(mysql_num_rows($sql_tit_asig) <= 12){
			
				while($row_tit_asig = mysql_fetch_array($sql_tit_asig)){
					...
					$row_tit_351 = mysql_fetch_array($sql_tit_351);
